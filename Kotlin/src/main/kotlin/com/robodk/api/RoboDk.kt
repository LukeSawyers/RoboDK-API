...
interface RoboDk {
     ...
     */
    fun addShape(
        trianglePoints: RealMatrix,
        addTo: Item? = null,
        shapeOverride: Boolean = false,
        color: Color? = null
    ): Item
     ...
     * @param addToRef If True, the points will be added as part of the object in the RoboDK item tree
     ...
     *     point normal and recalculate the normal vector on the surface projected.
     *
     * @returns The projected points as a list of points (empty matrix if failed).
     *
     */
    fun projectPoints(
        points: RealMatrix,
     ...
     *
     * @returns TODO: Document possible return values.
     */
     ...
     * If collision check is activated it returns the number of pairs of objects that are currently in a collision
     * state.
     *
     ...
     */
    fun setColor(itemList: List<Item>, colorList: List<Color>)

    /**
     * Show a list of objects or a robot link as collided (red) or as not collided (normal color).
     *
     * @param itemList List of items.
     * @param collidedList List of collided flags (True=show as collided).
     * @param robotLinkId Robot link ID, when applicable.
     */
    fun showAsCollided(itemList: List<Item>, collidedList: List<Boolean>, robotLinkId: List<Int>? = null)
    ...
    /**
     * Calibrate a Reference Frame given a list of points or joint values.
     * Important: If the robot is calibrated, provide joint values to maximize accuracy.
     *
    ...
    /**
     * Defines the name of the program when the program is generated.
     * It is also possible to specify the name of the post processor as well as the folder to save the program.
     * This method must be called before any program output is generated
     * (before any robot movement or other instruction).
     *
     ...
     */
    fun popupISO9283CubeProgram(robot: Item? = null): Item
